...
#define OURMODNAME   "slub_debug_eg"

MODULE_AUTHOR("Kaiwan NB");
MODULE_DESCRIPTION("SLUB debug : example usage");
...
{
	pr_info("** Test case :: OOB read underflow  dynamic-mem [func %s()] **\n", __func__);
	pr_debug("*p = 0x%lx\n", *p);
...
{
	pr_info("** Test case :: OOB read overflow dynamic-mem [func %s()] **\n", __func__);
	pr_debug("*p = 0x%lx\n", *p);
	pr_info("reading at kva 0x%lx+%+d : 0x%x\n", *p, sz, *(*p+sz));  // sz is passed as +ve
}
...
static void oob_read_overflow_compilemem(void)
{
	...
	pr_info("** Test case :: OOB read overflow compile-mem [func %s()] **\n", __func__);
	for (i = 0; i < 5; i++)
		arr[i] = i;
...
{
	pr_info("** Test case :: OOB write underflow  dynamic-mem [func %s()] **\n", __func__);
	pr_debug("*p = 0x%lx\n", *p);
	memset(*p+sz, 0xab, 32);  // sz is passed as -ve
}
...
static void oob_write_overflow_compilemem(void)
{
	...
	pr_info("** Test case :: OOB write overflow compile-mem [func %s()] **\n", __func__);
	//for (i = 0; i <= 5; i++) {
	for (i = 0; i <= 50; i++) {
...
static int __init slub_debug_eg_init(void)
{
	char *kp = NULL, *res;

	pr_debug("%s: inserted; testcase = %d\n", OURMODNAME, testcase);
	if (0 == testcase) {
		pr_info("%s: requires the module parameter 'testcase' to be passed. Aborting...\n",
			OURMODNAME);
		return -EINVAL;
	}
	kp = init();
	...
	case 1 : umr();
		break;
	case 2 : oob_write_overflow_compilemem();
		break;
	case 3 : oob_write_overflow_dynmem(&kp, 8);
		break;
	case 4 : oob_write_underflow(&kp, -8);
		break;
	case 5 : oob_read_overflow_compilemem();
		break;
	case 6 : oob_read_overflow_dynmem(&kp, NUM+8);
		break;
	case 7 : oob_read_underflow(&kp, -NUM-8);
		break;
	case 8 : uaf();
		break;
	case 9 : res = kmalloc(32, GFP_KERNEL);
		res = uar();
		pr_info(" res: %s\n", (char *)res);
		kfree(res);
		break;
	...
	case 11 : leak_simple();
		break;
	default: pr_info("Invalid testcase # %d passed, aborting...\n", testcase);
		break;
	}
...
static void __exit slub_debug_eg_exit(void)
{
	pr_debug("%s: removed\n", OURMODNAME);
}

module_init(slub_debug_eg_init);
module_exit(slub_debug_eg_exit);
